...
class CpuCluster(SubSystem):
	...
	def addL2(self, clk_domain):
		...
		self.L2monitor.trace = MemTraceProbe(trace_file = "monitor.ptrc.gz")
		self.toL2Bus.master = self.L2monitor.slave
		self.L2monitor.master = self.l2.cpu_side
		#self.toL2Bus.slave = self.L2monitor.master
		#membus.master = self.L2monitor.slave 
		#self.L2monitor.master = self.toL2Bus.slave
		#self.L2monitor.slave = membus.master
...
class ArmSESystem(System):
	def __init__(self, args, **kwargs):
		...
		#self.cpu_cluster.addMonitor(self.membus)
